KERNOBJS = \
	ai.o bio.o console.o display.o exec.o file.o fs.o ide.o ioapic.o kalloc.o kbd.o lapic.o \
  log.o main.o mp.o pipe.o proc.o sleeplock.o spinlock.o string.o swtch.o \
  syscall.o sysfile.o sysproc.o trapasm.o trap.o uart.o vectors.o vga.o vm.o \
#
...
_drawdemo: drawdemo.o $(DRAWLIB) $(ULIB) user.ld
	$(LD) $(LDFLAGS) -n -N -T user.ld -e main -Ttext 0x1000 -o $@ drawdemo.o $(DRAWLIB) $(ULIB)
	$(OBJDUMP) -S $@ > drawdemo.asm
	$(OBJDUMP) -t $@ | sed '1,/SYMBOL TABLE/d; s/ .* / /; /^$$/d' | sort > drawdemo.sym

_aidraw: aidraw.o $(DRAWLIB) $(ULIB) user.ld
	$(LD) $(LDFLAGS) -n -N -T user.ld -e main -Ttext 0x1000 -o $@ aidraw.o $(DRAWLIB) $(ULIB)
	$(OBJDUMP) -S $@ > aidraw.asm
	$(OBJDUMP) -t $@ | sed '1,/SYMBOL TABLE/d; s/ .* / /; /^$$/d' | sort > aidraw.sym
...
UPROGS= \
	...
	_rm _sh _stressfs _usertests _wc _zombie _prettyprint _imshow _drawdemo _aidraw\
#
...
qemu-ai: fs.img xv6.img
	$(QEMU) $(QEMUOPTS) -chardev socket,id=serial1,host=localhost,port=4444,server=on,wait=off -serial chardev:serial1

qemu-ai-debug: fs.img xv6.img
	$(QEMU) $(QEMUOPTS) -chardev socket,id=serial1,host=localhost,port=4444,server=on,wait=off -serial chardev:serial1 -monitor tcp:127.0.0.1:4445,server,nowait
